...
else is ordinary RAG. The same diagram is kept standalone at
[docs/architecture.mmd](docs/architecture.mmd) for editing; a test fails if the
...
There is also a **257-page technical study of this repository** —
...
[Results](#results); its Appendix H records the corrections that followed.
...
- The embedded index mode uses a linear scan. That is appropriate at this
  corpus size — **3,608 chunks**, against a 20,000-point advisory ceiling in
  `store/client.py` — and not for arbitrary scale. Past that, the same client
  factory points at a Qdrant server without touching calling code.
